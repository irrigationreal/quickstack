...
function forbidden(message = 'API key is not authorized to attach this service.') { return NextResponse.json({ status: 'error', message }, { status: 403 }); }
...
export async function POST(request: Request) {
    ...
    if (!parsed.success) return NextResponse.json({ status: 'error', message: 'Invalid service attach payload.' }, { status: 400 });
    const app = await appService.getById(parsed.data.appId);
    const service = await appService.getById(parsed.data.serviceId);
    ...
    const attachment = await serviceAttachmentService.attach({ ...parsed.data, actor: authenticated.auditActor });
    return NextResponse.json({ status: 'success', attachment });
}

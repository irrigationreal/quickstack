...
function forbidden(message = 'API key is not authorized to read storage snapshots for this app.') {
...
export async function GET(request: Request, { params }: { params: Promise<{ appId: string }> }) {
    ...
    return NextResponse.json({ status: 'success', appId: app.id, projectId: app.projectId, snapshots: storage.snapshots ?? [] });
}

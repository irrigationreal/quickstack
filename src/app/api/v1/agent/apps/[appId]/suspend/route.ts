...
function forbidden(message = 'API key is not authorized to suspend this app.') {
...
export async function POST(request: Request, { params }: { params: Promise<{ appId: string }> }) {
    ...
    if (!apiKeyService.hasScope(authenticated.apiKey, 'deploy:write')) {
        await auditService.recordBestEffort({
            ...
            action: 'AGENT_APP_SUSPEND_REQUESTED',
            outcome: 'DENIED',
            targetType: 'APP',
            targetId: appId,
            appId,
    ...
    const deployment = await deploymentService.setReplicasForDeployment(app.projectId, app.id, 0);
    if (app.replicas !== 0) {
        await appService.save({ id: app.id, replicas: 0 }, false);
    }
    ...
    await auditService.recordBestEffort({
        ...
        appId: app.id,
        appName: app.name,
        metadata: { previousReplicas, replicas: 0 },
    });

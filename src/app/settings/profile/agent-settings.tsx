...
import { QUICKSTACK_ASSET_PATHS } from "@/shared/model/quickdeploy-assets.model";
...
function buildAgentSetupScript(origin: string): string {
    const assetDownloads = QUICKSTACK_ASSET_PATHS
        .map((path) => `curl -fsSL ${origin}/agent/skills/quickstack/${path} -o ~/.claude/skills/quickstack/${path}`)
        .join('\n');

    return `# QuickStack agent setup for ${origin}\n# Paste the qstk_ API key from this page when prompted. It is saved outside projects.\n#\n# Agent instructions:\n# - If the user provides a qstk_ API key, save it with quickstack setup. Do not ask for it again unless saving fails.\n# - Store credentials only in ~/.quickstack/config.json or QUICKSTACK_URL/QUICKSTACK_API_KEY for CI.\n# - Never put API keys, registry passwords, env secrets, upload tokens, or kubeconfig in .quickstack, SKILL.md, or repo files.\n# - Use .quickstack only for secret-free deployment state like app id, project id, hostname, service root, mode, port, content hash, and image reference.\n# - For first deploys, run: quickstack launch \"$PWD\" --json\n# - For repeat deploys from a linked folder, run: quickstack deploy \"$PWD\" --json\n# - If the CLI returns structured questions, ask the user instead of guessing service topology, public hostname, raw endpoints, secrets, project, or app choices.\n\nset -euo pipefail\n\n# Install the server-matched QuickStack CLI and skill assets\nmkdir -p ~/.claude/skills/quickstack ~/.claude/commands ~/.quickstack\nchmod 700 ~/.quickstack\ncurl -fsSL ${origin}/api/cli/install | sh\n${assetDownloads}\n\n# Install slash-command wrapper so /quickstack works in Claude Code.\n# The command delegates to the installed CLI; it does not contain secrets.\ncat > ~/.claude/commands/quickstack.md <<'QUICKSTACK_COMMAND'\n---\ndescription: Run QuickStack agent workflows with the installed quickstack CLI\nargument-hint: [setup|launch|deploy|status|logs|secrets options]\ndisable-model-invocation: true\n---\n\nUse the installed QuickStack skill and quickstack CLI for this request.\n\nUser request:\n\n$ARGUMENTS\n\nPrefer quickstack ... over direct API calls. If the request includes a qstk_ API key, immediately save it with quickstack setup --url ${origin} --api-key <key> and do not ask for it again unless saving fails. Keep .quickstack secret-free; it is only deployment state. Relay CLI JSON questions/errors back to the user when deployment topology, secrets, domains, public endpoints, or project/app choices are ambiguous.\nQUICKSTACK_COMMAND\n\n# Save instance credentials for agents and the CLI. Do not put this in .quickstack or a repo.\nread -r -s -p \"QuickStack API key (qstk_...): \" QUICKSTACK_API_KEY\nprintf '\\n'\n~/.quickstack/bin/quickstack setup --url ${origin} --api-key \"$QUICKSTACK_API_KEY\"\nunset QUICKSTACK_API_KEY\n\n# Optional shell shortcut. Add this line to ~/.zshrc if you want it permanently.\nalias quickstack=\"$HOME/.quickstack/bin/quickstack\"\n~/.quickstack/bin/quickstack --help`;
}
            ...
            <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Copy a pasteable setup block that installs the QuickStack CLI and skill assets, tells the agent exactly where to save the qstk_ API key, saves the QuickStack URL/API key to ~/.quickstack/config.json with 0600 permissions, and adds a local quickstack CLI alias. Do not paste keys into SKILL.md, .quickstack, or project files.
                    </p>
                    {pluginConfig && (
                        <Button
                            variant="secondary"
